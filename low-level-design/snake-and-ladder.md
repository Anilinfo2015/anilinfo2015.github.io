...
Support two to eight players on numbered cells 1 through N, initially at position 0. A six-sided die advances the active player. Winning requires an exact landing on N; overshooting leaves the token unchanged but consumes the turn. Six grants no extra turn. Concurrent callers share one in-memory engine. Each turn takes O(1) time after board validation.
...
Acquire the game lock. Check version, terminal status, and active player before consulting the die. Require a result from 1 through 6; exceptions or invalid values produce `DieFailure` without consuming the turn. Compute `candidate = position + roll`. On overshoot, keep the old position and do not activate any jump there. Otherwise replace candidate with its jump destination when present.
