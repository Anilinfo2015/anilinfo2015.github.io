...
Reserve seats with expiring holds and reconcile payment results without double booking or claiming expired seats.
...
Hold one to ten seats for five minutes, initiate payment, and confirm all selected seats together. Store prices in integer minor units with one currency per booking. A hold is valid on the half-open interval [createdAt, expiresAt), using database time.

### Non-functional and execution constraints

Concurrent service instances use a transactional database. Booking work locks only the requested seats. Persist payment intent before calling the provider so a crash does not lose an accepted charge request.
...
For hold, `BookingService` deduplicates seat identifiers and locks requested `ShowSeat` rows in sorted order. Recheck show validity and occupancy. An expired hold owner is reclaimable; confirmed owners are not. Compute the quote from server prices, insert the `Booking`, and assign all seats in one transaction.
